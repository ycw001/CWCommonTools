//
//  CWCommonTools+Multimedia.m
//  CWCommonTools
//
//  Created by CW on 2020/12/16.
//

#import "CWCommonTools+Multimedia.h"
#import "CWCommonTools+Encrypt.h"
#import "CWCommonTools+FileHandle.h"

AVAudioPlayer *avPlayer = nil;
BOOL vibrateRepeat = false;  //标记是否循环震动

@implementation CWCommonTools (Multimedia)
...
- (AVAudioPlayer *)playMusicWithMusicFilePath:(NSString *)musicPath withRepeat:(BOOL)repeat withCategory:(AVAudioSessionCategory)audioSessionCategory {
    ...
    if (![[CWCommonTools sharedCWCommonTools] isLocalURLLink:musicPath]) {
        NSString *name = [[CWCommonTools sharedCWCommonTools] MD5EncryptWithString:musicPath withLowercase:true];
         NSURL *path = [[[CWCommonTools sharedCWCommonTools] createFileDirectoryWithType:kCWFileDirectoryTypeCaches andDirectoryName:@"music"] URLByAppendingPathComponent:name isDirectory:false];
        NSData * audioData = [NSData dataWithContentsOfURL:[NSURL URLWithString:musicPath]];
        [audioData writeToURL:path atomically:true];
        musicPath = path.path;
    }
    
    NSURL *musicUrl = [[CWCommonTools sharedCWCommonTools] urlCreatedByString:musicPath];
